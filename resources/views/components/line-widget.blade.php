...
<div x-data="{ open: false }" class="fixed bottom-4 right-4 z-[70] flex flex-col items-end gap-2">
    ...
    <div x-show="open" x-cloak x-transition
         ...
         class="w-72 overflow-hidden border border-brand-gray-border bg-white shadow-md">
    ...
    {{-- Toggle button --}}
    <button type="button" @click="$dispatch('close-mobile-menu'); open = !open" aria-label="Open chat"
            class="flex h-12 w-12 items-center justify-center rounded-full border border-brand-black bg-white text-brand-black transition-colors hover:bg-brand-black hover:text-white">
        <svg x-show="!open" class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
